...
Run() 시작 시 MemoryManager를 통해 Long-term Memory를 1회 로드한다.
...
LLMPlanner가 OpenAI API를 통해 LLM에 요청하고, 응답을 PlanResult로 파싱한다.

**결정 가능한 행동 유형 (ActionType):**
- `tool_call` — 특정 Tool을 호출하도록 지시
- `respond_directly` — Tool 없이 바로 응답 생성
- `summarize` — 이전 ToolResults를 종합해 최종 답변 생성
- `ask_user` — 사용자에게 추가 입력을 요청
- `finish` — loop 종료
...
ToolExecutor가 ToolRouter를 내부에서 사용해 Tool을 실행한다.
...
세션은 여러 Request를 포함할 수 있으며, InMemorySessionRepository 또는 RedisSessionRepository에 저장된다.
...
Agent가 참조할 수 있는 정보 저장소.

### Long-term Memory
Postgres(또는 InMemory)에 저장되는 기억. 태그 기반으로 검색하며 다음 요청에서도 참조 가능하다.
"지난번에 사용자가 선호한 조건" 같은 정보가 여기에 해당한다.
Runtime이 Run() 시작 시 LoadRelevantMemory로 조회하여 AgentState.RelevantMemories에 주입한다.
Run() 종료 후 호출자가 SaveMemory로 저장한다.

### MemoryManager
SessionRepository와 MemoryRepository를 단일 인터페이스로 캡슐화하는 파사드.
Runtime은 이 인터페이스만 의존하며, 구체 저장소를 직접 알지 않는다.

> 코드 위치: `internal/memory/`
...
> Phase 5 예정. 코드 위치: `internal/verifier/` (미구현)
...
> Phase 6 예정. 코드 위치: `internal/orchestration/task.go` (미구현)
...
ManagerAgent가 이 Workflow를 해석하고 실행 순서를 결정한다.

> Phase 6 예정. 코드 위치: `internal/orchestration/` (미구현)
