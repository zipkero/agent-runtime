...
planner, executor, tool router, state, memory, verifier, async task queue + worker 같은 runtime 구성요소를 직접 나누고 연결하면서, agent system이 실제로 어떻게 굴러가는지 손으로 체득하는 커리큘럼이다.
...
| AsyncTaskQueue + Worker | Phase 7 (HTTP API + async task 큐 + worker) |
